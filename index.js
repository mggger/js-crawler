...
const scrapeData = async (browser, url) => {
    ...
    const tocData = await page.evaluate(() => {
        ...
        return rows.map(row => {
            ...
            return { text: anchor.innerText, href: anchor.getAttribute('href') };
...
const mergeAndWriteData = async () => {
    ...
    for (let record of records) {
        ...
        if (url.includes('https://www.legislation.govt.nz/regulation/public/2000/0195/latest/whole.html')) {
            newDataset.push({ title, subTitle: title, url });
        } else {
